...
import testnuklear.IOUtil;

/**
 * Manages the layout and rendering of all nuklear windows
 */
public class Manager {
	...
	/**
	 * to render the nuklear windows
	 */
	private int vbo, vao, ebo, prog, vert_shdr, frag_shdr, uniform_tex, uniform_proj;
	...
	public Manager() {
		try {
			this.ttf = IOUtil.ioResourceToByteBuffer("font/zx-spectrum.ttf", 512 * 1024);
		} catch (IOException e) {
			throw new RuntimeException(e);
		}
	}

	public void init(Window window, Game game) {
		this.win = window.getID();
	...
	public GLFWScrollCallbackI getScrollCallBack() {
		return new GLFWScrollCallbackI() {
	...
	public GLFWCursorPosCallbackI getCursorPosCallBack() {
		return new GLFWCursorPosCallbackI() {
	...
	private NkContext setupWindow(Window w) {
...
//		menu.layout(window, ctx, 0, 0);
		debug.layout(window, ctx, 0, 0);
		playerGUI.layout(window, ctx, 0, 0);
